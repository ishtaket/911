...
| **False sense of biometric security**: auto-bootstrap silently downloaded an ECAPA "placeholder" URL that expects mel-features but gets raw PCM — embeddings = noise, cosine ≈ 0, any voice would pass as the owner. | `ModelBootstrapWorker` no longer includes ECAPA in the auto-target list. ECAPA stays MANUAL-only via Settings → Model downloads, where the user explicitly chooses to download and re-enrolls afterwards. Documented placeholder status in `ModelRegistry.ECAPA_TDNN_ONNX` kdoc. | `PCA-S-19` | `ModelBootstrapWorker.doWork`, `ModelRegistry.kt` |
| Auto-bootstrap fills the data partition or burns through cellular cap | `Constraints.setRequiresStorageNotLow(true) + setRequiresBatteryNotLow(true) + setRequiredNetworkType(UNMETERED)` by default; explicit `available < 2× model size` precheck inside the worker; bounded retry (`runAttemptCount >= 10 → Failure`) so a permanently-broken URL stops eating data | `PCA-S-20` `PCA-S-24` `PCA-S-29` | `ModelBootstrapWorker.doWork`, `ModelBootstrap.schedule` |
| Bridge open to entire LAN with no auth → any device burns subscription / reads transcripts | `server.py` default `--host` changed from `0.0.0.0` to `127.0.0.1`. Loopback-only is the new safe default; opt-in `--host 0.0.0.0` prints a WARNING log line. Termux installer already passes loopback; laptop variant docs updated to use specific LAN IP or SSH tunnel | `PCA-S-23` | `bridge/server.py`, `pca-android/README.md` |
| install-termux.sh = curl-pipe-bash anti-pattern with no integrity check | Header now documents both the curl-pipe and the safer clone+inspect+run flow; script refuses to run outside Termux (`/data/data/com.termux/files/usr` check) so a stray pipe into a regular Linux shell can't damage anything | `PCA-S-21` `PCA-S-22` | `bridge/install-termux.sh` |
| User manually downloads ECAPA in Settings without realising the default URL is a placeholder that produces noise embeddings | `ModelRow` for the ECAPA spec renders a tertiary-coloured warning describing the issue and pointing at PCA-S-19 in SECURITY.md | `PCA-S-30` | `ui/settings/SettingsScreen.kt` |
| Bootstrap-failure dashboard chip crashes with `MissingFormatArgumentException` when the failure reason contains a literal `%` (e.g. kernel "%2X" diagnostics) | Escape `%` → `%%` before passing to `stringResource` | `PCA-S-31` | `ui/dashboard/DashboardScreen.kt` |
| Runaway / compromised bridge fills `open_threads` with thousands of new rows per window | `WindowProcessor` caps `decision.openThreadsUpdate.new` at 32 per window; thread id/topic/context fields length-capped | `PCA-S-33` | `WindowProcessor.process`, `LlmDecisionBoundsTest` |
| Close-list with >999 ids crashes SQLite via the default `IN (...)` parameter limit | Chunked into batches of 900 via `chunked(SQLITE_IN_LIMIT)` | `PCA-S-34` | `WindowProcessor.process`, `LlmDecisionBoundsTest` |
| `decision.advice` / `decision.reason` arbitrarily long → DB row bloat + UI lag | Capped at 4096 / 1024 chars before insert; notification + history surface the truncated form | `PCA-S-35` | `WindowProcessor.process`, `LlmDecisionBoundsTest` |
| Compromised bridge responds `302 Location: http://attacker.com/decide` → OkHttp silently follows and exfiltrates the (anonymised but correlatable) window payload | Two distinct OkHttpClients via Hilt qualifier: the **default** one (used by HttpBridgeProvider) sets `followRedirects(false) + followSslRedirects(false)` — any 30x bubbles up to ProviderRouter as a failure. The `@ModelDownloadHttp` qualified client keeps redirects ON because HuggingFace `/resolve/main/` 302s to its CDN; URL gating in `ModelDownloader.isAcceptableModelUrl` rejects any redirected target that isn't https or LAN-cleartext, so a malicious redirect can't downgrade a public download to attacker-http | `PCA-S-36` | `di/AppModule.kt` (`@ModelDownloadHttp`), `ModelDownloader` |
| Future parallelised LLM dispatch would race the failure counter (`Int += 1` is read-modify-write, not atomic) and silently miss failures — bridge breaker never trips | Counter migrated to `AtomicInteger` (`incrementAndGet` / `set(0)`); single-threaded ticker path was safe but defensive coding for any future fan-out | `PCA-S-37` | `ProviderRouter.kt` |
| CSRF from a malicious website to a loopback bridge — fetch with credentials, no preflight, burns the user's subscription. FastAPI's strict Content-Type=application/json was incidental protection only | Explicit `CORSMiddleware(allow_origins=[])` denies every cross-origin request. Defense-in-depth middleware also refuses any incoming request that carries an Origin header (the Android app never sets one). | `PCA-S-38` | `bridge/server.py` |
| Supply chain — `GIT_TAG v1.7.1` is mutable; upstream could force-push a different commit under the same tag and CMake would happily fetch it | Documented (CMakeLists.txt comment) the hardened alternative of pinning by full 40-char commit SHA + `GIT_SHALLOW FALSE`. Tag form remains the default for routine maintenance; users who care about full supply-chain integrity can flip to SHA in a one-line edit. | `PCA-S-40` | `app/src/main/cpp/CMakeLists.txt` |
